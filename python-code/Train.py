...
def GetJsonFromRecords(fileName):
    jsonFileName = xmlToJson(fileName)
...
def main():
    ...
    recordToTest = "../data/smokers_surrogate_all.xml"
    df = GetJsonFromRecords(recordToTest)
    df = ColTransform(df)
    df["smoking_unknown"] = df["smoking_status"].apply(UnknownCol)

    df["description"] = df["description"].apply(TextProcess)

    X1, y1 = Split(df, 'description', 'smoking_unknown')
    tfidf_vect1 = TfidfVectorizer(use_idf= False,sublinear_tf= True,norm= 'l1',ngram_range= (1, 2),min_df= 3,max_features=7000,max_df=0.8)
    classifier1=GradientBoostingClassifier(min_samples_leaf= 1, min_samples_split= 20,max_features= 'auto',max_depth=1, learning_rate= 0.05,n_estimators=100,random_state=15325)

    val=1 # for keeping count of the pickle file
    ModelLR(X1, y1, tfidf_vect1, classifier1, val)
    # second layer of classification
    secondf = pd.DataFrame()
    secondf = df[df.smoking_unknown == 0]
    print(secondf.head())

    secondf["smoking_nonsmoker"] = secondf["smoking_status"].apply(NonSmokerCol)

    X2, y2 = Split(secondf, 'description', 'smoking_nonsmoker')
    print(secondf.head())
    tfidf_vect2 = TfidfVectorizer(use_idf= False,sublinear_tf= True,norm= 'l1',
                             ngram_range= (1, 2),min_df= 4,max_features=7000,max_df=0.8)

    classifier2 = GradientBoostingClassifier(min_samples_leaf= 1,min_samples_split= 20,max_features= 'auto',max_depth=1, learning_rate= 0.05,n_estimators=100,random_state=15325)
    ...
    X3, y3 = Split(thirdf, 'description', 'smoking_past')
